...
class GoogleimagesSpider(ImageSpider):
    ...
    def parse(self, response):
        ...
        for _ in range(self.MAX_IMAGES_PER_MEMBER):
            yield from self.parse_item(response.meta)

        # Fetch remaining.
        # TODO(Kagami): We can parallel that too.
        response.meta['_knet_parallel'] = False
    ...
    def save_item(self, response):
        ...
        if not response.meta['_knet_parallel']:
            return self.parse_item(response.meta)

    def errback_item(self, failure):
        if not failure.request.meta['_knet_parallel']:
